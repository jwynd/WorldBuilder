...
if(document.getElementById('size').value){
  ...
  let tokens = islandArea;

  // 1 <= limit <= tokens
  let limit = tokens / agents;

  // 1 <= octave <= 1000
  let octave = Math.pow(10, coastUniformity);

  // MountainAgent parameters

  // Controls the length of a mountain range
  let mountainTokens = 150;

  // Controls height of mountain peaks
  let maxPeak = 220;
  let minPeak = maxPeak * 0.5;

  // Controls how long an agent walks before turning
  let maxWalkTime = (1 - (squiggliness / 100)) * mountainTokens;
  let minWalkTime = maxWalkTime * 0.5;
